/**
 * Git Quest — Google Sheets recorder.
 *
 * Optional: appends one row per student when they press "Send progress" in
 * the game, with one column per level (stars out of 3).
 *
...
function testAppend() {
  var e = { postData: { contents: JSON.stringify({
    ...
    total: 5, maxTotal: 60, percent: 8.3,
    questions: [
      { id: "1-1", score: 3, max: 3, note: "1 cmds" },
      { id: "1-2", score: 2, max: 3, note: "4 cmds, 1 hint(s)" }
    ]
  })}};
